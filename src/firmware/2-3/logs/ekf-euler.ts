 ...
 * ROLL                  4     4   float   rad  Roll angle                     
 * PITCH                 8     4   float   rad  Pitch angle                    
 * YAW                  12     4   float   rad  Yaw angle (heading)            
 * ROLL_ACC             16     4   float   rad  1σ Roll angle accuracy            
 * PITCH_ACC            20     4   float   rad  1σ Pitch angle accuracy           
 * YAW_ACC              24     4   float   rad  1σ Yaw angle accuracy             
 * SOLUTION_STATUS      28     4  uint32     -  Global solution status         
 *           Total size 32
 * 
...
export const SBG_ECOM_LOG_EKF_EULER = (payload: Buffer): SBGParsedData => {
  ...
  const data = {
      ...
      rollAcceleration: payload.readFloatLE(16),
